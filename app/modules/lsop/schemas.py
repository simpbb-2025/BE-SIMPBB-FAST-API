...
class LampiranCreatePayload(BaseModel):
    nop: Optional[str] = Field(default=None, max_length=50)
    ...
    formulir_sesuai_ketentuan: Optional[bool] = None

    @model_validator(mode="after")
    def ensure_any_field(self) -> "LampiranCreatePayload":
...
class LampiranRecord(BaseModel):
    ...
    nop: Optional[str]
    jumlah_bangunan: Optional[int]
    bangunan_ke: Optional[int]

    jenis_penggunaan_bangunan: Optional[str]
    kondisi_bangunan: Optional[str]
    tahun_dibangun: Optional[int]
    tahun_direnovasi: Optional[int]
    luas_bangunan_m2: Optional[int]
    jumlah_lantai: Optional[int]
    daya_listrik_watt: Optional[int]

    jenis_konstruksi: Optional[str]
    jenis_atap: Optional[str]
    jenis_lantai: Optional[str]
    jenis_langit_langit: Optional[str]

    jumlah_ac: Optional[int]
    jumlah_ac_split: Optional[int]
    jumlah_ac_window: Optional[int]
    ac_sentral: Optional[bool]

    luas_kolam_renang_m2: Optional[int]
    kolam_renang_diplester: Optional[bool]
    kolam_renang_dengan_pelapis: Optional[bool]

    tenis_lampu_beton: Optional[int]
    tenis_lampu_aspal: Optional[int]
    tenis_lampu_tanah_liat: Optional[int]

    tenis_tanpa_lampu_beton: Optional[int]
    tenis_tanpa_lampu_aspal: Optional[int]
    tenis_tanpa_lampu_tanah_liat: Optional[int]

    jumlah_lift_penumpang: Optional[int]
    jumlah_lift_kapsul: Optional[int]
    jumlah_lift_barang: Optional[int]

    jumlah_tangga_berjalan_lebar_kurang_80_cm: Optional[int]
    jumlah_tangga_berjalan_lebar_lebih_80_cm: Optional[int]

    panjang_pagar_meter: Optional[int]
    pagar_bahan_baja_besi: Optional[bool]
    pagar_bahan_bata_batako: Optional[bool]

    pemadam_hydrant: Optional[bool]
    pemadam_sprinkler: Optional[bool]
    pemadam_fire_alarm: Optional[bool]

    kedalaman_sumur_artesis_meter: Optional[int]

    kelas_bangunan_perkantoran: Optional[str]
    kelas_bangunan_ruko: Optional[str]
    kelas_bangunan_rumah_sakit: Optional[str]
    luas_ruang_kamar_ac_sentral_m2: Optional[int]
    luas_ruang_lain_ac_sentral_m2: Optional[int]
    kelas_bangunan_olahraga: Optional[str]

    jenis_hotel: Optional[str]
    bintang_hotel: Optional[str]
    jumlah_kamar_hotel: Optional[int]
    luas_ruang_kamar_hotel_ac_sentral_m2: Optional[int]
    luas_ruang_lain_hotel_ac_sentral_m2: Optional[int]

    kelas_bangunan_parkir: Optional[str]
    kelas_bangunan_apartemen: Optional[str]
    jumlah_kamar_apartemen: Optional[int]

    letak_tangki_minyak: Optional[str]
    kapasitas_tangki_minyak_liter: Optional[int]

    kelas_bangunan_sekolah: Optional[str]

    foto_objek_pajak: Optional[str]
    formulir_sesuai_ketentuan: Optional[bool]
